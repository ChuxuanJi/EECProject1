...
groups_1_T<-read.delim('group1_1_T.txt',header = T)
colnames(groups_1_T)[1]<-'samples'





nmds12_1<-merge(nmds12_1,groups_1_T,by='samples')
...
ggplot() +
  ...
  geom_point(data = nmds12_1,aes(MDS1,MDS2,color=as.factor(group)),size=4)+
...
ggsave('nmds_envfit_1_T.pdf',width = 8,height = 6)
...
ggplot() +
  #geom_text_repel(data = st,aes(RDA1,RDA2,label=row.names(st)),size=4)+#Show a Square
  geom_point(data = nmds12_1,aes(MDS1,MDS2,color=as.factor(group)),size=4)+
  stat_ellipse(data = nmds12_1,aes(MDS1,MDS2,color=as.factor(group)), level = 0.95)+
  scale_color_npg()+
  geom_hline(yintercept=0,linetype=2) + 
  geom_vline(xintercept=0,linetype=2)+
  labs(x='NMDS1',y="NMDS2",title = stress)+
...
ggsave('nmds_without_envfit_1_T.pdf',width = 8,height = 6)
