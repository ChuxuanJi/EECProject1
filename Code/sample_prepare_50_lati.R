...
colnames(otu2)[1]<-'OTU'
otu2<-otu2[rowSums(otu2[,-1]>50),]
#拟合
otu3<-otu2 %>%group_by(OTU) %>% summarise_all(sum)
otu4<-otu3[,c(T,colSums(otu3[,-1])>0)]
write.table(otu4,'filter_otu_50.txt',row.names = F,sep = '\t',quote = F)
...
write.table(mygroup,'group1_50_10_lati.txt',row.names = F,sep = '\t',quote = F)